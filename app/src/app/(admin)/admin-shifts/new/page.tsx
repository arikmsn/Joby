...
export default function AdminCreateShiftPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch("/api/admin/employers?limit=100", {
  ...
  const canSubmit =
    !!form.employer_id && !!form.title && !!form.role_tag && !!form.address && !!form.start_at && !!form.end_at && !!form.pay_rate;
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              <label htmlFor="employer_id" className="block text-sm font-medium text-foreground mb-1">{t("admin.shifts.employer")}</label>
              <select id="employer_id" className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-colors" value={form.employer_id} onChange={(e) => set("employer_id", e.target.value)}>
                <option value="" disabled>{t("admin.shifts.employer")}</option>
                {employers.map((emp) => (
                  <option key={emp.id} value={emp.id}>{emp.business_name || emp.full_name}</option>
                ))}
              </select>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button onClick={() => submit(true)} loading={loading} disabled={!canSubmit}>
                {t("shift.publish")}
              </Button>
              <Button variant="secondary" onClick={() => submit(false)} loading={loading} disabled={!canSubmit}>
                {t("shift.save_draft")}
              </Button>
