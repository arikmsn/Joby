...
export default function AdminEmployersPage() {
  ...
  function load() {
    ...
    if (q) params.set("q", q);
    fetch(`/api/admin/employers?${params.toString()}`, {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setShowCreate(false);
      setForm({ phone: "", full_name: "", business_name: "", business_type: "", city: "", address: "" });
      load();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">{t("nav.employers")}</h1>
      ...
      {showCreate && (
        <Card className="space-y-3">
          <h2 className="text-sm font-semibold text-foreground">{t("admin.employers.create")}</h2>
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-semibold text-foreground">{row.business_name || row.full_name}</h3>
                  <p className="text-sm text-foreground-secondary mt-0.5">
                    {row.full_name} · <span dir="ltr">{row.phone}</span>
                    {row.city ? ` · ${row.city}` : ""}
                  </p>
                </div>
