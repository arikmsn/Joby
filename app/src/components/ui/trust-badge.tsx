...
export function TrustBadge({ score, totalShifts, size = "sm" }: TrustBadgeProps) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 font-semibold tabular-nums transition-colors duration-150",
        colorClass,
        size === "sm" ? "text-xs" : "text-sm"
      )}
    >
      <Shield className={cn("shrink-0", size === "sm" ? "h-3 w-3" : "h-3.5 w-3.5")} />
      {label}
    </span>
  );
}
